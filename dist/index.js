...
const handleSpawn = (debug, fileName, port, filePath) => {
  ...
  console.log(debug, fileName, port, filePath);
    ...
    if (stderr.toString().includes("Request_Id_On_The_Left")) {
      ...
      debug
        ...
        : //TODO add Correct error logging url request/ response/
          cleanExit(
            `Error Processing Request (please open an issue https://github.com/Danny-Dasilva/CycleTLS/issues/new/choose) -> ${stderr}`,
            false
...
class Golang extends events_1.EventEmitter {
  ...
  spawnServer() {
    ...
    const executableFilename =
      PLATFORM_BINARIES[process.platform]?.[os_1.default.arch()];
    if (!executableFilename) {
      cleanExit(
        new Error(
          `Unsupported architecture ${os_1.default.arch()} for ${
            process.platform
          }`
        )
      );
      ...
      const CycleTLS = (() => {
        const CycleTLS = async (url, options, method = "get") => {
          return new Promise((resolveRequest, rejectRequest) => {
            const requestId = `${url}#${Date.now()}-${Math.floor(
              1000 * Math.random()
            )}`;
            //set default options
            options ??= {};
            ...
            instance.once(requestId, (response) => {
              if (response.method === "error") {
                ...
                resolveRequest({
                  ...
                  raw: () => promise,
                  json: () =>
                    promise.then((data) => JSON.parse(data.toString())),
                  text: (enc) => promise.then((data) => data.toString(enc)),
                });
              }
            });
          });
...
///Users/ivaninja/dev/cf-clearance-scraper/node_modules/cycletls/dist/index-mac-arm64
///Users/ivaninja/dev/CycleTLS/dist/index-mac-arm64
